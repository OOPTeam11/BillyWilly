...
public:
	bool create(IDirect3DDevice9* pDevice, int index, D3DXCOLOR color = d3d::WHITE)
	{
		if (NULL == pDevice)
			return false;
	...
	bool hasIntersected(CSphere& ball)
	{
		...
		if (pow(abs(center_x - position.x), 2) + pow(abs(center_z - position.z), 2) < pow(1.999999999 * M_RADIUS, 2)) {
			...
			if (ball_v_result>800) {
				sndPlaySound("c:\\부딪히는소리5.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 5단계
			}
			else if (ball_v_result > 600)
			{
				sndPlaySound("c:\\부딪히는소리4.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 4단계
			}
			else if (ball_v_result > 400)
			{
				sndPlaySound("c:\\부딪히는소리3.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 3단계
			}
			else if (ball_v_result > 200)
			{
				sndPlaySound("c:\\부딪히는소리2.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 2단계
			}
			else
			{
				sndPlaySound("c:\\부딪히는소리1.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 1단계
			}
...
public:
	...
	bool hasIntersected(CSphere& ball)
	{
		...
		if (m_width > m_depth) {	//벽의 가로길이가 세로길이보다 길 떄
	...
	void hitBy(CSphere& ball)
	{
		...
		if (hasIntersected(ball)) {
			...
			if (ball_v_result>800) {
				sndPlaySound("c:\\벽3.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 3단계
			}
			else if (ball_v_result > 400)
			{
				sndPlaySound("c:\\벽2.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 2단계
			}
			else
			{
				sndPlaySound("c:\\벽1.wav", SND_ASYNC | SND_NOSTOP);                    // 부딪히는 소리 재생         // 1단계
			}
